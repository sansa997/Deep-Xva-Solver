...
class Swap(Equation):
    def __init__(self,eqn_config):
        super(Swap, self).__init__(eqn_config)
        self.rate_model = eqn_config.rate_model
        self.b = eqn_config.b;
        self.a = eqn_config.a;
        self.schedule_float =  eqn_config.schedule_float;
        self.schedule_fix =  eqn_config.schedule_fix;
        self.r_init = np.ones(self.dim) * eqn_config.r_init  # initial value of r, the floating rate
        self.sigma = eqn_config.sigma
        self.r_fix = eqn_config.r_fixed      
        self.risk_free = eqn_config.risk_free
        self.useExplict = False #whether to use explict formula to evaluate dynamics of r
    ...
    def sample(self, num_sample):      
        ...
        #    case "CIR":
        for i in range(self.num_time_interval):
            r_sample[:, :, i + 1] = r_sample[:, :, i] + self.a * (self.b - r_sample[:, :, i]) *self.delta_t  +\
                self.sigma * tf.math.sqrt(tf.math.abs(tf.cast(r_sample[:, :, i], tf.float64))) *\
                    tf.math.sqrt(tf.cast(self.delta_t, tf.float64))* dw_sample[:, :, i]         
      
        
        return dw_sample, r_sample   

  
    def f_tf(self, t, r, y, z):
        return -self.risk_free * y
   
    def g_tf(self, t, r_float):
        
        self.schedule_float = tf.cast(self.schedule_float, tf.float64)
        
        #return tf.reduce_sum(r_float*tf.exp(-self.risk_free*self.schedule_float), axis=2) -\
            #tf.cast(self.r_fix*np.exp(-self.risk_free*self.schedule_fix), tf.float64)
        return tf.reduce_sum(r_float*tf.exp(-self.risk_free*self.schedule_float), axis=2)
#/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

class BarrierOption(Equation):
    def __init__(self,eqn_config):
        super(BarrierOption, self).__init__(eqn_config)
        self.strike = eqn_config.strike
    ...
    def g_tf(self, t, x):
        #barrier_check = tf.math.minimum(x,1)>self.barrier #stessa dimensione di x, 1 se è ok, 0 se è fuori dalla barriera
        barrier_check = tf.math.greater(x, self.barrier)
        barrier_check = tf.cast(barrier_check, tf.float64)
        check = tf.reduce_sum(barrier_check, axis=2) #sommiamo nel tempo (ogni scenario, ogni dimensione di w) --> dim_output = num_sample x self.dim
        a= tf.maximum(x[:,:,-1] - self.strike, 0)*(check // self.num_time_interval)  #se c'è uno 0, mettiamo a 0 l'output --> dim_output = num_sample x self.dim
        return a
        # return x - self.strike
